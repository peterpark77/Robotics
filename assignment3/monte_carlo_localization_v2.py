...
def print_particle(particle):
    print "x-coordinate ", particle.x
    print "y-coordinate ", particle.y
...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        #exponentiate the terms using numerical stability

        b = min(errors)
        errors_negative = [(b-x) for x in errors]
        exponentiate = [exp(error) for error in errors_negative]
        self.weights = [i/sum(exponentiate) for i in exponentiate]
        N_eff = int(1/sum([i*i for i in self.weights]))



        #error = [exp(-error) for error in errors]
        #self.weights = [i/sum(error) for i in error]

        #self.weights = [i/sum(exponentiate) for i in exponentiate]



        #errors = [i / sum(errors) for i in errors]
        #self.weights = errors
        #N_eff = int(1/sum([i*i for i in self.weights]))
    ...
    def resample(self):
        ...
        new_particles = []
        r = np.random.uniform(0, 1.0/len(self.particles))
        idx = 0
        c = self.weights[0]
        for i in range(len(self.particles)):
            u = r + (i /float(len(self.particles)))
            while u > c:
                idx = idx + 1
                c = c + self.weights[idx]
            p = self.particles[idx]
            new_particle = Particle(p.id,p.x, p.y, p.theta)
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        sample_ranges,sample_angles = self.subsample_laser_scan(laser_scan_msg)

        # TODO: simulate a laser scan using one of the methods of this class

        ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, sample_angles, laser_scan_msg.range_min, laser_scan_msg.range_max)
        ...
        for i in range(len(ranges)):
            norm_error += (ranges[i] - sample_ranges[i])**2
