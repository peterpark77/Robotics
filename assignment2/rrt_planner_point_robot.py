...
#################UNIFORM##########################
   ...
   # x = math.floor(random.uniform(0,XMAX))
   # y = math.floor(random.uniform(0,YMAX))
...
def rrt_search(G, tx, ty):
  ...
  # while not abs(pointPointDistance(vertices[closestPointToPoint(G, [tx, ty])], [tx, ty]) < SMALLSTEP):
    while not outer_break:
        ...
        for i in vertices:
            ...
            for rect in obstacles:
                if not lineHitsRect(q_nearest, q_new, rect) and q_new not in vertices:
                    hit += 1
            if hit == len(obstacles):
                node_index = pointToVertex(q_new)
                G[edges].append([closest, node_index])
                G[nodes].append(node_index)
                if abs(pointPointDistance(vertices[node_index], [tx,ty])) < 5:
                    outer_break = True
                    break
        redraw()
        canvas.events()
    ...
    redraw()
    canvas.events()
    print("rrt_path length is" , path_length)
